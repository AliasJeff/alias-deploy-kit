...
def build_prompt(instruction: str, inp: str = "") -> str:
    if inp.strip():
        return PROMPT_TEMPLATE_WITH_INPUT.format(instruction=instruction, input=inp)
...
def tokenize_fn(examples, tokenizer):
    ...
    for instruction, inp, output in zip(
        ...
        # mask prompt tokens in labels (only train on the response)
...
def main():
    ...
    train_ds = train_ds.map(
        tok_fn, batched=True, batch_size=500,
